...
def parse_xml(args):
    ...
    for obj in root.findall('object'):
        ...
        bbox = [
            int(round(float(bnd_box.find('xmin').text))),
            int(round(float(bnd_box.find('ymin').text))),
            int(round(float(bnd_box.find('xmax').text))),
            int(round(float(bnd_box.find('ymax').text)))
        ]
        if difficult:
...
def cvt_annotations(devkit_path, out_file):
    annotations = []
    filelist = osp.join(devkit_path, f'images.txt')
    if not osp.isfile(filelist):
        print(f'filelist does not exist: {filelist}, ')
        return
    img_names = mmcv.list_from_file(filelist)
    xml_paths = [
        osp.join(devkit_path, 'Annotations', f'{img_name}.xml')
...
def cvt_to_coco_json(annotations):
    ...
    for category_id, name in enumerate(['person']):#voc_classes()):
        category_item = dict()
...
def main():
    ...
    if args.out_format == 'coco':
        out_fmt = '.json'
    cvt_annotations(devkit_path, osp.join(out_dir, 'annotations' + out_fmt))
    print('Done!')
